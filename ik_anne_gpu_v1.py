...
class IK_anne_gpu():
    ...
    def fit(self,
        ...
        # shape=(t, psi)
        self._center_index_list = np.concatenate([np.random.choice(X.shape[0], size=(1, self._psi), replace=False) for _ in range(self._t)], axis=0)
    ...
    def transform(self,
        ...
        for _batch_num in tqdm.trange(batch_num):
            ...
            for i in range(self._t):
                ...
                output[start_idx+batch_size:end_idx+batch_size, :][torch.arange(batch_cuda.shape[0]), (p2ns_index + i * self._psi)] = 1
